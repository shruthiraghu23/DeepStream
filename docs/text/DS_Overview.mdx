...
The core SDK consists of several hardware accelerator plugins that use accelerators such as VIC, GPU, DLA, NVDEC and NVENC. By performing all the compute heavy operations in a dedicated accelerator, DeepStream can achieve highest performance for video analytic applications. One of the key capabilities of DeepStream is secure bi-directional communication between edge and cloud. DeepStream ships with several out of the box security protocols such as SASL/Plain authentication using username/password and 2-way TLS authentication. To learn more about these security features, read the [IoT](/text/DS_IoT) chapter. To learn more about bi-directional capabilities, see the [Bidirectional Messaging](/text/DS_IoT#bidirectional-messaging) section in this guide.
...
* After decoding, there is an optional image pre-processing step where the input image can be pre-processed before inference. The pre-processing can be image dewarping or color space conversion. [Gst-nvdewarper](/text/DS_plugin_gst-nvdewarper) plugin can dewarp the image from a fisheye or 360 degree camera. [Gst-nvvideoconvert](/text/DS_plugin_gst-nvvideoconvert) plugin can perform color format conversion on the frame. These plugins use GPU or VIC (vision image compositor).
* The next step is to batch the frames for optimal inference performance. Batching is done using the [Gst-nvstreammux](/text/DS_plugin_gst-nvstreammux) plugin.
* Once frames are batched, it is sent for inference. The inference can be done using TensorRT, NVIDIA’s inference accelerator runtime or can be done in the native framework such as TensorFlow or `PyTorch` using Triton inference server. Native TensorRT inference is performed using [Gst-nvinfer](/text/DS_plugin_gst-nvinfer) plugin and inference using Triton is done using [Gst-nvinferserver](/text/DS_plugin_gst-nvinferserver) plugin.
* After inference, the next step could involve tracking the object. There are several built-in reference trackers in the SDK, ranging from high performance to high accuracy. Object tracking is performed using the [Gst-nvtracker](/text/DS_plugin_gst-nvtracker) plugin.
* For creating visualization artifacts such as bounding boxes, segmentation masks, labels there is a visualization plugin called [Gst-nvdsosd](/text/DS_plugin_gst-nvdsosd).
* Finally to output the results, DeepStream presents various options: render the output with the bounding boxes on the screen, save the output to the local disk, stream out over RTSP or just send the metadata to the cloud. For sending metadata to the cloud, DeepStream uses [Gst-nvmsgconv](/text/DS_plugin_gst-nvmsgconv) and [Gst-nvmsgbroker](/text/DS_plugin_gst-nvmsgbroker) plugin. [Gst-nvmsgconv](/text/DS_plugin_gst-nvmsgconv) converts the metadata into schema payload and [Gst-nvmsgbroker](/text/DS_plugin_gst-nvmsgbroker) establishes the connection to the cloud and sends the telemetry data. There are several built-in broker protocols such as Kafka, MQTT, AMQP and Azure IoT. Custom broker adapters can be created.
