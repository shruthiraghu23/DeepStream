...
| nvmodelmux | Model load/unload/update, stream-to-model routing, model status, and application metadata (see [Gst-nvmodelmux](/text/DS_plugin_gst-nvmodelmux)) |
...
<Note>
These endpoints control the [Gst-nvmodelmux](/text/DS_plugin_gst-nvmodelmux) element's MODEL plane and STREAM-ROUTING plane. All bodies use the `{"key": "model"|"stream", "value": {...}}` envelope.
</Note>
